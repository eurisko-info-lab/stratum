...
object StratumRepo:
        ...
        sub match
          ...
          case "record" =>
            ...
            record(
              ...
              source,
              opts.get("catalogue-root").map(resolve(root, _)).getOrElse(source),
          ...
          case "status" =>
            status(
              ...
              opts.get("catalogue-root").map(resolve(root, _)).getOrElse(resolve(root, opts.getOrElse("source", ".")))
            )
    ...
    open(dir) match
      ...
      case Right(cas) =>
        ...
        if !Files.isDirectory(source) then CommandResult.fail(s"source directory does not exist: $source")
          ...
          val catalogue = LanguageCatalogue.load(catalogueRoot) match
            case Left(error) => return CommandResult.fail(error)
            case Right(value) => value
          val structured = files.map { file =>
            val relative = unix(source.relativize(file))
            LanguageCatalogue.structure(catalogueRoot, relative, Files.readAllBytes(file), catalogue, cas)
              .map(relative -> _)
  ...
  private def status(dir: Path, branch: String, source: Path, catalogueRoot: Path): CommandResult =
    open(dir) match
      case Left(error) => CommandResult.fail(error)
      case Right(cas) =>
