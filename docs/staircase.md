...
11. add its new objects to the parent materialization image
12. commit the sources, expected digest, application artifact and verdict
13. publish the child image and tag the commit `foundation/Fn`
...
The Git commit records the implementation event. The canonical foundation
change is the authoritative semantic event, and the [materialization
image](images.md) carries the accepted state forward without replaying earlier
commits.
