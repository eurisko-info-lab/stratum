...
CI looks for the exact commit image first. On a miss it requires the exact
immediate-parent image and advances only the checked-out commit. It never scans
for an older ancestor or rebuilds intervening commits. A trusted push publishes
the immutable child image. Pull requests keep their new layer local to the
workflow.
