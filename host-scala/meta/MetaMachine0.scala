...
object MetaMachine0:
  ...
  def derive(
    ...
    try
      ...
      derived match
        case Right(canon) => okWithSemantics(value, state, trace, engine.evaluationStack)
        case Left(message) => errorWithSemantics("derivation-graph", message, state, trace, engine.evaluationStack)
    catch
      case f: MetaFail          =>
        val state = engine.stateSnapshot
        errorWithSemantics(f.kind, f.message, state, engine.traceEntries, engine.evaluationStack)
      case _: StackOverflowError =>
        val state = engine.stateSnapshot
        errorWithSemantics("depth-exhausted", "derivation exceeded host stack", state, engine.traceEntries, engine.evaluationStack)
