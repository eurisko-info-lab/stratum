...
object FoundationCommands:
  ...
  def run(root: Path, args: Vector[String]): CommandResult =
    Cli.positional(args).headOption match
      ...
      case Some("derive-change")     => deriveChange(root, Cli.options(args))
      case Some("derive-successor")  => deriveSuccessor(root, Cli.options(args))
      case Some("reconstruct")       => reconstruct(root, Cli.options(args))
      case Some("attest")            => attest(root, Cli.options(args))
      case Some("report")            => report(root, Cli.options(args))
      ...
      case (Some(specPath), Some(outPath)) =>
        readCanonSource(root, specPath) match
          ...
          case Right(spec) =>
            ...
            Journal.createDirectories(closureDir)
                  ...
                  case Some(appTemplate) =>
                    resolveTemplate(root, cas, appTemplate) match
                      ...
                      case Right(app) =>
                        ...
                        Journal.write(outDir.resolve("foundation.canon"), foundationArtifact.bytes)
                        Journal.write(outDir.resolve("application.canon"), appArtifact.bytes)
                        Journal.writeString(outDir.resolve("digest.txt"), foundationDigest.hex + "\n")
  ...
  private def deleteRecursively(p: Path): Unit =
    if Files.exists(p) then
      ...
      try
        stream.sorted(java.util.Comparator.reverseOrder()).iterator().asScala.foreach(Journal.delete)
                ...
                case Right(digests) =>
                  ...
                  result match
                    ...
                    case Right(attestation) =>
                      ...
                      // Writing the attestation directly keeps the two-host
                      // comparison independent of any build tool's logging.
   ...
   * verdict.
   *
   * Two independent implementations of MetaMachine0 agree exactly when this
   * report is byte identical, which covers values, evidence and deterministic
   * resource accounting.
   */
  /**
   * Runs a judgment of a foundation's own program over quoted arguments.
   *
   ...
   *
   * The diff is computed by the predecessor's own `DiffFields` judgment, so the
   * change is a semantic artifact rather than a host construction.
   */
  private def deriveChange(root: Path, opts: Map[String, String]): CommandResult =
    (opts.get("predecessor"), opts.get("successor")) match
      case (Some(previous), Some(next)) =>
        val predecessorDir = root.resolve(previous)
        val successorDir = root.resolve(next)
        (Cli.loadFoundation(root, predecessorDir), Cli.loadFoundation(root, successorDir)) match
          case (Left(m), _) => CommandResult.fail(s"predecessor: $m")
          case (_, Left(m)) => CommandResult.fail(s"successor: $m")
          case (Right(predecessor), Right(successor)) =>
            val name = field(successor.foundation, "name").getOrElse(Canon.S(""))
            ...
            runIn(
              ...
              case Right(derivation) =>
                opts.get("out").foreach { out =>
                  val target = root.resolve(out)
                  Option(target.getParent).foreach(Journal.createDirectories(_))
                  val header = s"; Generated by `foundation derive-change`. Do not edit by hand.\n"
                  Journal.writeString(target, header + CanonText.pretty(derivation) + "\n")
                }
                CommandResult.ok(
   ...
   *
   * `DeriveSuccessor` receives the predecessor reference and the change only.
   * No successor manifest is supplied, so a match with the committed digest is
   * evidence that F_n genuinely builds F_{n+1}.
   */
  private def deriveSuccessor(root: Path, opts: Map[String, String]): CommandResult =
    (opts.get("predecessor"), opts.get("derivation")) match
      case (Some(previous), Some(derivationPath)) =>
        val predecessorDir = root.resolve(previous)
        Cli.loadFoundation(root, predecessorDir) match
          case Left(m) => CommandResult.fail(s"predecessor: $m")
          case Right(predecessor) =>
            readCanonSource(root, derivationPath) match
              case Left(m) => CommandResult.fail(m)
              case Right(derivation) =>
                runIn(
                  predecessor,
                  ...
                  case Left(m) => CommandResult.fail(m)
                  case Right(Canon.Node("derived", Vector(Canon.R(derived)))) =>
                    val predecessorName =
                      field(predecessor.foundation, "name").collect { case Canon.S(s) => s }.getOrElse("?")
                    val head = Vector(
                      s"predecessor $predecessorName",
                      s"derived ${derived.hex}"
                    )
                    opts.get("expect") match
                      case None => CommandResult.okLines(head)
                      case Some(expected) =>
                        val digestFile = root.resolve(expected).resolve("digest.txt")
                        if !Files.exists(digestFile) then
                          CommandResult(1, head :+ s"expected foundation has no digest.txt")
                        else
                          val committed = Files.readString(digestFile).trim
                          val matched = committed == derived.hex
                          CommandResult(
                            if matched then 0 else 1,
                            head :+ s"matches $expected ${if matched then "yes" else "no"}"
                          )
                  case Right(other) => CommandResult(1, Vector(CanonText.write(other)))
      case _ =>
        CommandResult.fail(
          "usage: foundation derive-successor --predecessor <dir> --derivation <file> [--expect <dir>]"
        )

  private def report(root: Path, opts: Map[String, String]): CommandResult =
    opts.get("dir") match
      case None => CommandResult.fail("usage: foundation report --dir <dir> [--out <file>]")
      case Some(dir) =>
        val d = root.resolve(dir)
        Cli.loadFoundation(root, d) match
          case Left(m) => CommandResult.fail(m)
          case Right(f) =>
            val seed = f.foundationDigest.hex
            val caps = stratum.cap.Capabilities.standard(f.cas, d, seed)
            val checks = f.checks.map {
              case check @ Canon.Node("check", Canon.Sym(name) +: goal +: _ +: rest) =>
                val budget = rest.headOption.flatMap(Budget.fromCanon(_).toOption).getOrElse(f.budget)
                val verdict = MetaMachine0.derive(f.program, f.cas, f.kernel, budget, goal, caps)
                Right(Canon.node("check", Canon.Sym(name), Canon.R(Canon.digest(verdict))))
              case other => Left(s"malformed check: ${CanonText.write(other)}")
            }
            checks.collectFirst { case Left(m) => m } match
              case Some(m) => CommandResult.fail(m)
              case None =>
                val value = Canon.node(
                  "derivation-report",
                  Canon.node("foundation", Canon.R(f.foundationDigest)),
                  Canon.node("checks", Canon.L(checks.collect { case Right(c) => c }))
                )
                val lines = Vector(CanonText.write(value), s"report ${Canon.digest(value).hex}")
                opts.get("out").foreach { out =>
                  val target = root.resolve(out)
                  Option(target.getParent).foreach(Journal.createDirectories(_))
                  Journal.writeString(target, lines.mkString("", "\n", "\n"))
